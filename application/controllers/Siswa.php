...
class siswa extends CI_Controller {
	...
	public function create()
    {
        ...
        // 2. Tambahkan pesan 'is_unique' untuk NIS dan Username
        $this->form_validation->set_rules('nis', 'nis', 'required|trim|is_unique[tb_siswa.nis]', [
            'required' => 'NIS harus di isi!',
            'is_unique' => 'NIS ini sudah terdaftar!' // Notifikasi duplikat
        ]);
        $this->form_validation->set_rules('username', 'Username', 'required|trim|is_unique[tb_siswa.username]', [
            'required' => 'Username harus di isi!',
            'is_unique' => 'Username ini sudah digunakan!' // Notifikasi duplikat
        ]);

        $this->form_validation->set_rules('password', 'Password', 'required|trim', ['required' => 'Password harus di isi!']);

        // 3. Kembalikan struktur IF/ELSE
        if ($this->form_validation->run() != false) {
            // Jika validasi SUKSES

            // Ambil data (Ambil HANYA setelah validasi sukses)
            $nama       = htmlspecialchars($this->input->post('nama', TRUE));
            $nis        = htmlspecialchars($this->input->post('nis', TRUE));
            $kelas      = htmlspecialchars($this->input->post('kelas', TRUE));
            $username   = htmlspecialchars($this->input->post('username', TRUE));
            $password   = htmlspecialchars($this->input->post('password', TRUE)); // Sebaiknya di-hash
            ...
            $data = array(
                ...
                'id_kelas' => $kelas,
                'username' => $username,
                'password' => $password, // Jangan lupa password
            );
            ...
            // Jika validasi GAGAL (misal: NIS duplikat atau ada yg kosong)
            // Tampilkan lagi form-nya. Notifikasi akan muncul otomatis
            // jika Anda menggunakan form_error() di view.
