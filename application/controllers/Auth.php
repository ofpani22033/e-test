    ...
    public function index()
    {
        ...
        if ($this->form_validation->run() != false) {
            ...
            if ($admin && $password == $admin['password']) {
                ...
                $data_session = array(
                    'id'        => $data->id, // Untuk Admin, 'id' sudah benar
                    'nama'      => $data->nama_user,
            ...
            // Cek Password Guru
            } else if ($guru && $password == $guru['password']) {
                
                $data = $this->m_data->edit_data($where, 'tb_guru')->row();
                $data_session = array(
                    // --- PERBAIKAN DI SINI ---
                    'id_guru'   => $data->id_guru, // Ganti 'id' menjadi 'id_guru'
                    'nama'      => $data->nama_guru,
                    'username'  => $data->username,
                    'status'    => 'guru_login'
                );
            ...
            // Cek Password Siswa
            } else if ($siswa && $password == $siswa['password']) {
                
                $data = $this->m_data->edit_data($where, 'tb_siswa')->row();
                $data_session = array(
                    // --- PERBAIKAN DI SINI ---
                    'id_siswa'  => $data->id_siswa, // Ganti 'id' menjadi 'id_siswa'
                    'nis'       => $data->nis,
