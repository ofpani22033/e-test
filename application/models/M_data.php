...
class M_data extends CI_Model
{
	...
	public function insert_multiple()
	{
		...
		$this->db->insert_batch('tb_hasil_ujian', $entri);
		return true;
	}
	...
	public function UpdateNilai2($id_peserta, $data)
	{
		...
		$this->db->update('tb_hasil_ujian', $data);
	}
    ...
    public function simpan_jawaban_satu($id_peserta, $id_soal, $jawaban)
    {
        // 1. Cek tabel tb_jawaban (apakah siswa ini sudah pernah jawab soal ini?)
        $this->db->where('id_peserta', $id_peserta);
        $this->db->where('id_soal_ujian', $id_soal);
        $query = $this->db->get('tb_jawaban');

        if($query->num_rows() > 0){
            // Kalo sudah ada -> UPDATE jawabannya
            $data = array('jawaban' => $jawaban);
            $this->db->where('id_peserta', $id_peserta);
            $this->db->where('id_soal_ujian', $id_soal);
            $this->db->update('tb_jawaban', $data);
        } else {
            // Kalo belum ada -> INSERT jawaban baru
            $data = array(
